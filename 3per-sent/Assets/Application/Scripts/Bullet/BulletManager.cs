	...
	void Update () {
		...
		if (restShotCount > 0) {
			...
			while (restShotCount > 0 && restInterval <= 0) {
				...
				//オレオレ仕様.悪いと思っている.
				if (_shotAngle >= 360) {
					shotAngle = this.transform.rotation.eulerAngles.z + _shotAngle - 360;
				}
				...
				for (int i = 0; i < lineCount; i++) {
					...
					Bullet bullet = ObjectPool.instance.GetOnlyGameObject(bulletPrefab).GetComponent<Bullet>();
					if (bulletType == BulletType.RandomDiffusionBullets) {
						shotSpeed = _shotSpeed + Random.Range(randomRangeMin, randomRangeMax);
					}

					bullet.gameObject.tag = this.tag;
					...
					if (bulletType == BulletType.NWayBullets
						...
						if (bulletType == BulletType.AimNWayBullets
					    	|| bulletType == BulletType.AimRandomNWayBullets
						    || bulletType == BulletType.GuidedMissile) {
							shotAngle = bullet.SetTarget(TagUtil.GetHomingActorTag(tag), guideTime);
						}
